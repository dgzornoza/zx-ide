...
@injectable()
export class AttachProjectSpritesCmd extends Command<unknown> {
  ...
  public async execute(..._params: unknown[]): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(vscode.l10n.t('Error opening Extract Sprites: {0}', String(error)));
    }
  }
  ...
  private async createWebViewPanel(): Promise<vscode.WebviewPanel> {
    ...
    const panel = vscode.window.createWebviewPanel(
      'zxide.attachProjectSprites',
      vscode.l10n.t('Extract sprites'),
    ...
    panel.webview.html = await WebviewHelpers.buildWebviewHtml({
      ...
      htmlPageName: 'extract-sprites.html',
      locale,
    });
