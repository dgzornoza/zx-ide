...
# Agent instructions for zx-ide web-client
...
Los siguientes skills están disponibles para este proyecto (se encuentran en `.ai/skills`). Cárgalos antes de trabajar en los temas relacionados:

- **vue-best-practices**: MUST be used for Vue.js tasks. Recomienda usar Composition API con `<script setup>` y TypeScript. Cargar para `.vue` files, composables, Vue Router, Pinia o trabajo con Vite + Vue. Ver [../skills/vue-best-practices/SKILL.md](../skills/vue-best-practices/SKILL.md) para detalles y referencias.

Si aparecen nuevos skills relacionados aplicables a este proyecto, añádelos a la carpeta `.ai/skills` y actualiza esta sección.
...
| File | Responsibility |
| --- | --- |
| `useExtractGraphics.ts` (en `src/extract-graphics/composables/`) | Main state machine: source files, tile extraction, sprite actions, VS Code bridge messaging, file generation |
| `codeGenerators.ts` (en `src/extract-graphics/composables/`) | Pure functions that produce C header (`.h`) and Z88DK assembly (`.asm`) source code from tile bitmask data |
| `graphicsMapValidation.ts` (en `src/extract-graphics/composables/`) | Validation logic for graphics map data |
...
Messages exchanged with the extension are defined in the **shared** DTOs at `shared/extract-graphics/extract-graphics-dtos.ts` (relative to repo root):
